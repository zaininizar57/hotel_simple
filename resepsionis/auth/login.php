<?php 
    ...
    if (isset($_POST['login'])) {
        ...
        if (strlen($error) > 0) {
            ...
            $sql = "SELECT * FROM users WHERE nama_lengkap = '" . $data['username'] . "' AND role_id = 2";
            $result = $conn->query($sql);
            if (strlen(mysqli_error($conn)) > 0) {
                header('Location: ?error_login=error:500');
            }else if(mysqli_num_rows($result) < 1){
                header('Location: ?error_login=resepsionis tidak di temukan');
            }else{
                ...
                if (password_verify($data['password'], $password)) {
                    ...
                    header('Location: ../resepsionis_panel.php');
                }
